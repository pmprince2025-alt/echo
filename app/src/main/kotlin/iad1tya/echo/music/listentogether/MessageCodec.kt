...
import prince.sonic.music.listentogether.proto.Listentogether
...
class MessageCodec(
    ...
    fun encode(msgType: String, payload: Any?): ByteArray = encodeProtobuf(msgType, payload)

    fun decode(data: ByteArray): Pair<String, ByteArray> = decodeProtobuf(data)
    ...
    private fun encodeProtobuf(msgType: String, payload: Any?): ByteArray {
        ...
        val envelope =
            Listentogether.Envelope.newBuilder()
                .setType(msgType)
    ...
    private fun toProtoMessage(payload: Any): MessageLite {
        return when (payload) {
            is CreateRoomPayload ->
                Listentogether.CreateRoomPayload.newBuilder()
                    .setUsername(payload.username)
                    .build()

            is JoinRoomPayload ->
                Listentogether.JoinRoomPayload.newBuilder()
                    .setRoomCode(payload.roomCode)
                    .setUsername(payload.username)
                    .build()

            is ApproveJoinPayload ->
                Listentogether.ApproveJoinPayload.newBuilder()
                    .setUserId(payload.userId)
                    .build()

            is RejectJoinPayload ->
                Listentogether.RejectJoinPayload.newBuilder()
                    .setUserId(payload.userId)
            ...
            is PlaybackActionPayload -> {
                val builder =
                    Listentogether.PlaybackActionPayload.newBuilder()
                ...
                payload.queue?.forEach { track -> builder.addQueue(trackInfoToProto(track)) }

                builder.build()
            }

            is BufferReadyPayload ->
                Listentogether.BufferReadyPayload.newBuilder()
                    .setTrackId(payload.trackId)
                    .build()

            is KickUserPayload ->
                Listentogether.KickUserPayload.newBuilder()
                    .setUserId(payload.userId)
                    .setReason(payload.reason ?: "")
                    .build()

            is SuggestTrackPayload ->
                Listentogether.SuggestTrackPayload.newBuilder()
                    .setTrackInfo(trackInfoToProto(payload.trackInfo))
                    .build()

            is ApproveSuggestionPayload ->
                Listentogether.ApproveSuggestionPayload.newBuilder()
                    .setSuggestionId(payload.suggestionId)
                    .build()

            is RejectSuggestionPayload ->
                Listentogether.RejectSuggestionPayload.newBuilder()
                    .setSuggestionId(payload.suggestionId)
                    .setReason(payload.reason ?: "")
                    .build()

            is ReconnectPayload ->
                Listentogether.ReconnectPayload.newBuilder()
                    .setSessionToken(payload.sessionToken)
                    .build()

            is TransferHostPayload ->
                Listentogether.TransferHostPayload.newBuilder()
